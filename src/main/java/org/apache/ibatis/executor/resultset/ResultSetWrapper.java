...
/**
 * java.sql.ResultSet 的 包装器
 * 可以理解成 ResultSet 的工具类，提供给 DefaultResultSetHandler 使用
 * @author Iwao AVE!
 */
public class ResultSetWrapper {

  // ResultSet 对象，被包装的 ResultSet 对象
   ...
   *
   * @param propertyType
   *          the property type  java类型
   * @param columnName
   *          the column name  字段名称
   *
   * @return the type handler  类型处理器对象
   */
  public TypeHandler<?> getTypeHandler(Class<?> propertyType, String columnName) {
    ...
    // 如果获取不到，则进行查找（基于 propertyType + jdbcType 进行查找）
    if (handler == null) {
      // 获取当前字段 JdbcType 类型
      ...
      // 如果获取不到，则再次进行查找
      if (handler == null || handler instanceof UnknownTypeHandler) { // 如果获取不到，则基于 javaType + jdbcType 进行查找。其中，javaType 使用 classNames 中的类型。通过 #resolveClass(String className) 方法，获得对应的类
      ...
      // 缓存 TypeHandler 对象 到 typeHandlerMap 中
  ...
  private void loadMappedAndUnmappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
    ...
    // 通过#getMapKey(ResultMap resultMap, String columnPrefix) 方法，获得缓存的 KEY
  ...
  public Set<String> getMappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
    ...
    if (mappedColumnNames == null) {
      // 初始化 有 mapped 和无 mapped的字段的名字数组
